...
(async ({ current_script, facet, facet_export, facet_load_error }) => { try {  // facet begin
    ...
    const default_epsilon = 10 * Number.EPSILON;


    // may throw an error
    const number_comparator = (a, b) => (+a - +b);

    // this function is expensive...
    function equal_within_espilon(a, b, epsilon=default_epsilon) {
    ...
    // this is expensive...
    function stats_equal_within_epsilon(stats1, stats2, epsilon=default_epsilon) {
        if (typeof epsilon !== 'number' || epsilon <= 0) {
            throw new Error('epsilon must be a positive number');
        }
    ...
    function vec_distance_squared_between(dim, vec1, vec2) {
        let d2 = 0;
        for (let i = 0; i < dim; i++) {
            d2 += (vec1[i] - vec2[i])**2;
        }
    ...
    // this function is expensive...
    function vec_equal_within_espilon(dim, vec1, vec2, epsilon=default_epsilon) {
    ...
    function vec_get_stats(dim, values) {
        const value_count = values.length;
        if (value_count <= 0) {
            return undefined;
        } else {
    ...
    function vec_stats_equal_within_epsilon(dim, stats1, stats2, epsilon=default_epsilon) {
        if (typeof epsilon !== 'number' || epsilon <= 0) {
            throw new Error('epsilon must be a positive number');
        }
        // if a stats value is undefined, then the stats
    ...
    facet_export({
        default_epsilon,
        number_comparator,
        equal_within_espilon,
        get_stats,
        stats_equal_within_epsilon,
        vec_distance_squared_between,
        vec_equal_within_espilon,
        vec_make_zero,
        vec_get_stats,
