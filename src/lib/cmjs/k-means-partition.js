...
(async ({ current_script, facet, facet_export, facet_load_error }) => { try {  // facet begin
    ...
    const default_value_variance = 0;


    // === "GOODNESS OF FIT" FUNCTIONS ===

    function partition_score(partition) {
    ...
    function centers_too_close(partition) {
        if (partition.k < 2) {
    ...
    function generate_intitial_partition(k, data, epsilon=default_epsilon) {
    ...
    function partition_by_centers(partition) {
        for (const s of partition.sets) {
            s.members = [];
        }
    ...
    function recalculate_stats(partition) {
        let changed = false;
        const centers = partition.sets.filter(s => s.stats).map(s => s.stats.center);
    ...
    function prune_empty_sets(partition) {
    ...
    facet_export({
        default_value_variance,
        partition_score,
        centers_too_close,
        Partition,
        generate_intitial_partition,
        partition_by_centers,
        recalculate_stats,
        prune_empty_sets,
    });
