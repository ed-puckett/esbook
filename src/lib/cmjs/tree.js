...
(async ({ current_script, facet, facet_export, facet_load_error }) => { try {  // facet begin
    ...
    const max_by_depth_row_node_count = 2**26;
    const max_format_overall_width    = 10000;

    const {
        number_comparator,
        get_stats,
    } = await facet('lib/cmjs/tree-data.js');


    // === TOP-LEVEL SUPPORT FUNCTIONS ===

    function make_tree_node(s=null, additional_properties) {
        return {
    ...
    // larger tree).
    function walk_tree(root, options) {
        options = options ?? {};
    ...
    function tree_child0_sn(parent_sn, alphabet_size) {
    ...
    function get_tree_alphabet(root) {
        const acc = {};
    ...
    function tree_by_depth(root, depth, alphabet) {
    ...
    function format_tree(root, value_getter, depth) {
    ...
    class MultiSampleTree extends Tree {
        // May throw an error.
    ...
    facet_export({
        max_by_depth_row_node_count,
        max_format_overall_width,
        make_tree_node,
        is_leaf_node,
        clone_tree,
        walk_tree,
